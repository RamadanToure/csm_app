...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						...
						@endif
						@if(in_array('exporter_expediteur',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" >
							<div><label for="labelInput" class="form-label">Type d'expéditeur</label>
								{!! Form::select('type_expe',trans('entite.type_expediteur'),session('type_expeSess'),["id"=>"type_expe","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('expediteur.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("expediteur/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("expediteur/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"expediteur?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("expediteur/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("expediteur/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/expediteur/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'expediteur/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'expediteur/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'expediteur/'+id,type : 'GET',dataType : 'html',
