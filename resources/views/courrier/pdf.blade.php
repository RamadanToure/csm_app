...
@if(count($list) != 0)
	...
	<table class="table" style="font-size:15px; width:100%;">
		<tr>
			<!-- <th class="th" >{!!trans('data.id_cour')!!}</th> -->
		...
		<tbody>{{$i = 1}}
			@foreach($list as $listgiwu)
				<tr style="background-color : <?php if ($i % 2 == 0) {echo '#ffffff';$i++;}else{echo trans("data.styleLignePdf");$i++;} ?>;">
					<!-- <td class="td">{{$listgiwu->id_cour}}</td> -->
					...
					<td class="td">{{trans('entite.priorite_courrier')[$listgiwu->priorite_cour]}}</td>
