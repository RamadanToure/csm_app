<title>{{Config('app.name') }} | Liste des directions</title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
{!!trans('data.stylePdf')!!} 
<div class="footer"><i>{!!trans('data.signaturePdf')!!} <span class="pagenum"></span> </i></div>

@if(count($list) != 0)
	<div><h3 style="text-align:center;">Liste des directions<br>
		@if(!empty($_GET['query']))
	...
	<table class="table" style="font-size:15px; width:100%;">
		<tr>
			<th class="th" >{!!trans('data.code_direc')!!}</th>
			<th class="th" >{!!trans('data.lib_direc')!!}</th>
		...
		<tbody>{{$i = 1}}
			@foreach($list as $listgiwu)
				<tr style="background-color : <?php if ($i % 2 == 0) {echo '#ffffff';$i++;}else{echo trans("data.styleLignePdf");$i++;} ?>;">
					<td class="td">{{$listgiwu->code_direc}}</td>
					<td class="td">{{$listgiwu->lib_direc}}</td>
