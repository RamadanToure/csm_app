	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre.' ('.$nbreCourrier.')'}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('exporter_courrier',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					...
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4"> 
							<div><label for="labelInput" class="form-label"> Liste des courriers </label>
								{!! Form::select('lisTypeCour',trans('entite.listTypeCourrier') ,session('lisTypeCourSess'),["id"=>"lisTypeCour","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
						<!--end col Date-->
						<div class="col-xxl-3 col-md-4"> 
							<div><label for="labelInput" class="form-label">Date réception d&eacute;but et fin</label>
								<div class='input-group'>
									{!! Form::text('created_atdeb',date('01/m/Y'),['id'=>'created_atdeb','onchange'=>'funcRecher()','class'=>'form-control','autocomplete'=>'off']) !!}
									<span class='input-group-text'> &agrave; </span>
									{!! Form::text('created_atfin',date('d/m/Y'),['id'=>'created_atfin','onchange'=>'funcRecher()','class'=>'form-control','autocomplete'=>'off']) !!}
								</div>
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('cons.listcourrieratraiter.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
    <!-- action -->
    <div>
        <div class="modal fade bs-example-modal-center" data-bs-backdrop="static" id="kt_transfert_4" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" role="dialog" >
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("listcourrieratraiter/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("listcourrieratraiter/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"listcourrieratraiter?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("listcourrieratraiter/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("listcourrieratraiter/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/listcourrieratraiter/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-transfert', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("courrier/AffichePopTransfert/") }}/'+id,type : 'GET',dataType : 'html',
				success : function(code_html, statut){$("#kt_transfert_4 .modal-dialog").html(code_html);$("#kt_transfert_4").modal('show');}
			});
		});
		$(document).on('click', '.btn-traiter', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("courrier/AffichePopTraiter/") }}/'+id,type : 'GET',dataType : 'html',
				success : function(code_html, statut){$("#kt_transfert_4 .modal-dialog").html(code_html);$("#kt_transfert_4").modal('show');}
			});
		});
	</script>
