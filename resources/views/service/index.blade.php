...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_service',session('InfosAction')))
							<button type='button' class='btn btn-primary btn-label right btn-add rounded-pill'><i class='ri-add-line label-icon align-middle fs-16 ms-2 rounded-pill'></i> Ajouter</button>
						@endif
						@if(in_array('exporter_service',session('InfosAction')))
					...
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" >
							<div><label for="labelInput" class="form-label">Liste des Directions</label>
								<?php $addUse = array(''=>'Selectionnez un element'); $listid_direc = $addUse + $listid_direc->toArray();?>
								{!! Form::select('id_direc',$listid_direc ,session('id_direcSess'),["id"=>"id_direc","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
						<!--end col-->
						<!-- <div class="col-xxl-3 col-md-4" > 
							<div><label for="labelInput" class="form-label">Liste des Responsable</label>
								<?php //$addUse = array(''=>'Selectionnez un element'); $listrespo_id = $addUse + $listrespo_id->toArray();?>
								{!! Form::select('respo_id',$listrespo_id ,session('respo_idSess'),["id"=>"respo_id","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div> -->
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('service.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("service/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("service/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"service?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("service/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("service/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/service/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'service/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'service/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'service/'+id,type : 'GET',dataType : 'html',
