		...
		<div class="card">
			...
			<div class="card-body"><p class="text-muted"></p>
				<div class="live-preview"><strong><div class="msgAjouter"></div></strong>
					<form action="{{route('courrier.update',$item->id_cour)}}" method="post" id="form" class="row g-3 needs-validation" novalidate enctype='multipart/form-data'>
						@csrf()
						@method('PATCH')
							<div class="row">
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::text('ref_cour',$item->ref_cour,["id"=>"ref_cour","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Reference" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="date_rece" class="form-label">{!!trans('data.date_rece')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::date('date_rece',$item->date_rece,["id"=>"date_rece","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date Reception" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="date_limite" class="form-label">{!!trans('data.date_limite')!!} </label>
									{!! Form::date('date_limite',$item->date_limite,["id"=>"date_limite","class"=>"form-control"  ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date limite" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::select('expe_id',$listexpe_id ,$item->expe_id,["id"=>"expe_id","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="sujet_cour" class="form-label">{!!trans('data.sujet_cour')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::text('sujet_cour',$item->sujet_cour,["id"=>"sujet_cour","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Sujet" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="priorite_cour" class="form-label">{!!trans('data.priorite_cour')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::select('priorite_cour',trans('entite.priorite_courrier'),$item->priorite_cour,["id"=>"priorite_cour","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
							...
							<div class="col-md-6">
								<div class="mb-3">
									<label for="piece_jointe_cour" class="form-label">{!!trans('data.piece_jointe_cour').' '.$item->piece_jointe_cour!!} <strong style='color: red;'> *</strong></label>
									<input class="form-control" type="file" id="piece_jointe_cour" name="piece_jointe_cour" <?php echo isset($item->piece_jointe_cour) ? '' : 'required'; ?> >
								</div>
							</div>
							<div class="col-md-12">
								<div class="mb-3">
									<label for="commentaire_cour" class="form-label">{!!trans('data.commentaire_cour')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::textarea('commentaire_cour',$item->commentaire_cour,["id"=>"commentaire_cour","class"=>"form-control","style"=>"height: 100px;"  ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Commentaires" ]) !!}
								</div>
							</div>
							<div class="col-12">
								<div class="text-end">
