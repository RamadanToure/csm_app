		...
		<div class="card">
			...
			<div class="card-body"><p class="text-muted"></p>
				<div class="live-preview"><strong><div class="msgAjouter"></div></strong>
					<form action="{{route('carriere.store')}}" method="post" id="form" class="row g-3 needs-validation" novalidate >
						@csrf()
							<div class="row">
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::select('type_fonct',trans('entite.type_destinataire'),null,["id"=>"type_destina","class"=>"form-select" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="id_fonct" class="form-label">{!!trans('data.id_fonct')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::select('id_fonct',[],null,["id"=>"id_desti","class"=>"form-select allselect","required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="date_debut_carr" class="form-label">{!!trans('data.date_debut_carr')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::date('date_debut_carr',date('Y-m-d'),["id"=>"date_debut_carr","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date debut" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="date_fin_carr" class="form-label">{!!trans('data.date_fin_carr')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::date('date_fin_carr',date('Y-m-d'),["id"=>"date_fin_carr","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date fin" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="salaire_carr" class="form-label">{!!trans('data.salaire_carr')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::number('salaire_carr','',["id"=>"salaire_carr","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Salaire" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::select('id_occupant',$listid_occupant ,null,["id"=>"id_occupant","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-12">
								<div class="text-end">
									<a href="{{route('carriere.index')}}" class="btn btn-outline-dark waves-effect mr-10 rounded-pill">Fermer</a>
									@if(in_array('add_carriere',session('InfosAction')))
