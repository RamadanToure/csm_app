                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-shrink-0"> 
                        @if(in_array('add_role',session('InfosAction')))
                            <a class="btn btn-primary btn-label right rounded-pill" href="{{route('role.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2 rounded-pill"></i>Ajouter</a>
                        @endif
                    </div>
                </div>
            </div>
...
@section('JS_content')
    ...
    <script type="text/javascript">
        ...
        function funcRecher(){
            ...
            return $.ajax({
                url: '{{ url("/role/")}}',data: filtreData,type: 'GET',
        ...
        $(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("role/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
				success : function(code_html, statut){$("#kt_delete_4 .modal-dialog").html(code_html);$("#kt_delete_4").modal('show');}
			});
		});
        $(document).on('click', '.btn-action', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("role/AffichePopAction/") }}/'+id,type : 'GET',dataType : 'html',
