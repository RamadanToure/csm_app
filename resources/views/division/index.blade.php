...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_division',session('InfosAction')))
							<button type='button' class='btn btn-primary btn-label right btn-add rounded-pill'><i class='ri-add-line label-icon align-middle fs-16 ms-2'></i> Ajouter</button>
						@endif
						@if(in_array('exporter_division',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4" > 
							<div><label for="labelInput" class="form-label">Liste des Services</label>
								<?php $addUse = array(''=>'Selectionnez un element'); $listid_serv = $addUse + $listid_serv->toArray();?>
								{!! Form::select('id_serv',$listid_serv ,session('id_servSess'),["id"=>"id_serv","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
						<!--end col-->
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('division.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("division/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("division/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"division?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("division/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("division/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/division/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-add', function () {
			$.ajax({url : 'division/create',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-modifier', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'division/'+id+'/edit',type : 'GET',dataType : 'html',
		...
		$(document).on('click', '#btn-supprimer', function () {
			...
			if(id == ''){
				...
				$.ajax({url : 'division/'+id,type : 'GET',dataType : 'html',
